...
def add_features(df):
  df['comments'] = df['comments'].apply(remove_html).apply(remove_accented_chars).apply(remove_tabs).apply(remove_blanks).apply(remove_digits).apply(ascii_to_string).apply(remove_special_characters).apply(remove_url).apply(remove_email_address).apply(expand_contractions).apply(to_lower).apply(remove_all_punctuation)
  return df
...
print(" ------------------- clean data -------------------")
train = parallelize_dataframe(data_raw, add_features)
train.to_csv("/home/sandipan/Documents/deep_learning/sentiment_analysis/data/sentiment_all_clean_multi.csv")
